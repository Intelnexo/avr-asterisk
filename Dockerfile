...
RUN set -ex; \
    ...
    apt-get install -y --no-install-recommends \
        ...
        wget subversion bzip2 patch file curl ca-certificates \
        ...
        libasound2-dev libncurses5-dev libtinfo5 \
        libavcodec-dev libavformat-dev libswresample-dev \
        libspeex-dev libspeexdsp-dev libspandsp-dev libogg-dev; \
    cd /usr/src; \
    ...
    ./configure --without-jansson-bundled --with-pjproject-bundled --disable-xmldoc --without-libxslt --without-dahdi ; \
...
RUN mkdir -p /etc/asterisk/tenants && \
    chmod 755 /etc/asterisk/tenants

# Increase system limits for high call volume
